...
class Blink {
    ...
    async diagnosticDebug() {
        ...
        for (const doorbell of homescreen.doorbells || []) {
            anonMap.set(doorbell?.id, 6000 + curr);
            anonMap.set(doorbell?.name, 'Doorbell ' + curr);
            anonMap.set(doorbell?.serial, 'E000000' + curr);
            curr++;
        }
        ...
        if (homescreen) {
            ...
            for (const doorbell of homescreen.doorbells) {
                ...
                // log('getCameraStatus()');
                // await anonymize(this.blinkAPI.getCameraStatus(doorbell.network_id, doorbell.id, 0)); - Fails
                log('getDevice()');
